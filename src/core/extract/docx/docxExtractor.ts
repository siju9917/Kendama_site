...
export function parseDocumentXml(xml: string): DocxParagraph[] {
  ...
  while ((m = paraRe.exec(xml)) !== null) {
    ...
    while ((t = textRe.exec(body)) !== null) {
      textParts.push(decodeXmlEntities(t[1]));
    }
    if (/<w:tab\b/.test(body)) {
      // A tab inside a paragraph: treat as a space.
    }
...
function paragraphsToRawLines(paragraphs: ReadonlyArray<DocxParagraph>): RawLine[] {
  return paragraphs
    ...
    .map((p) => {
      // Translate Word heading styles into a "bold + larger font" hint so the
      // heading detector picks them up.
      ...
      const lineText = p.isList ? `- ${p.text}` : p.text;
