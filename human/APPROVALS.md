...
- **Phase 0 (zero-cost):** ~~Factory builds the OpenAPI rule engine + engine adapter in TypeScript. This can start before the human sets up VS Code Marketplace publisher registration.~~ **COMPLETED 2026-06-06 (ahead of proposal auto-proceed).** Engine is at `products/openapi-lens/`. **401/401 tests** passing (14 adversarial hardening rounds + 1 Swagger 2.0 parser bug fix; all OapiSchema fields covered at all levels: top-level body, per-property, items, parameter, parameter.items; doubly-nested array items diffing with depth guard; response items null-transition classify fixes (rounds 13–14, all three nesting levels covered)). Full type system, 80+ breaking-change rules. Zero VS Code dependency yet.
