...
- **Phase 0 (zero-cost):** ~~Factory builds the OpenAPI rule engine + engine adapter in TypeScript. This can start before the human sets up VS Code Marketplace publisher registration.~~ **COMPLETED 2026-06-06 (ahead of proposal auto-proceed).** Engine is at `products/openapi-lens/`. **1183/1183 tests** passing (engine Phase 0 + D5/D6 Phase 1 VS Code extension + 95 adversarial bug-hunt rounds covering all constraint scopes, HTTP methods, parameter locations, and null-transition/value-change/isolation scenarios; full type system, 84+ breaking-change rules, response-header-enum-changed + response-header-nullable-changed, all Phase 1 extension + Terraform classifier tests). Zero VS Code dependency for the engine; D5/D6 VS Code extension Phase 1 DONE.
