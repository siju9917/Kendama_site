...
| 2026-06-06 (round 18) | Domain-Expert Critic (#5) / Correctness Critic (#1) | Added: the `additionalProperties` field must be checked at ALL three schema nesting levels — top-level body schema (diffAdditionalProperties), per-property level (diffPropertyAdditionalProperties), AND items level (diffSchemaItems). The items-level gap is especially common: REST APIs that accept or return arrays of objects frequently use `additionalProperties: false` on the item schema to reject/guarantee clean objects; a change to this field was invisible. Probe: a POST endpoint with a request body `{type: array, items: {type: object, additionalProperties: false}}`; removing `additionalProperties: false` from items must produce an INFO finding (constraint relaxed); adding it must produce BREAKING. | openapi-lens `diffSchemaItems`: `additionalProperties` was compared at top-level (round 16) and property-level (round 16) but NOT at items-level. Adding `additionalProperties: false` to request array items (server now rejects elements with extra properties) was completely invisible. Added detection inside `diffSchemaItems`; 2 new OapiChangeType values, 4 classify rules, +8 tests (5.7.5 round 18, 2026-06-06 continuation). |
| 2026-06-06 (round 19) | Correctness Critic (#1) / Domain-Expert Critic (#5) | Added: enum array comparisons in diff functions MUST be order-insensitive. OpenAPI enum values form a **set** of allowed values; the order within the YAML/JSON array is not semantically significant. `JSON.stringify(["a","b"]) !== JSON.stringify(["b","a"])` even though both represent the same set. A spec that merely reorders enum values (common in auto-generated or machine-reformatted specs) will produce a spurious INFO event with empty `added=[]` and `removed=[]` lists — noise that degrades user trust. Fix pattern: `enumSetsEqual()` helper using `Set(a.map(JSON.stringify))` for order-insensitive comparison. This must be applied at ALL enum comparison sites: top-level body schema, property-level, items-level, parameter-schema, and parameter-items. The `deepEqual` (order-sensitive) function is NOT correct for set-valued fields. | openapi-lens `diff.ts`: 5 `!deepEqual(x.enum, y.enum)` sites used the order-sensitive JSON.stringify comparison. Reordering enum values (e.g., `["asc","desc"]` → `["desc","asc"]`) produced a spurious INFO event. Added `enumSetsEqual()` helper; replaced all 5 sites. Genuine enum additions/removals still produce correct events. +7 tests (5 reorder-no-event + 2 genuine-change-still-detected) (5.7.5 round 19, 2026-06-06 continuation). |
| 2026-06-06 (round 20) | Correctness Critic (#1) checklist | Added: after adding a new field comparison inside `diffSchemaItems` (or any diff function that has a `if (bEntity && cEntity)` guard for "both must exist" comparisons), immediately check whether the new field belongs inside or outside the guard. Fields that report a change ON an existing schema (additionalProperties, readOnly, writeOnly) must be inside the guard to avoid spurious double-events when the entity is newly added. Probe: add a response items schema with the new field set — must produce only the type-changed event (before=null), NOT an additional field-changed event. | openapi-lens `diffSchemaItems`: round 18 added `additionalProperties` comparison OUTSIDE the `if (bItems && cItems)` guard that protects readOnly/writeOnly. When items were newly added (bItems=undefined), `bAP = true` (default) vs `cAP = false` (new schema) emitted a spurious `response-schema-items-additional-properties-changed` event alongside the expected `items-type-changed (null→object)`. Fix: merged additionalProperties into the guard, consistent with round 10 semantics. +3 tests. **476/476 tests.** |
| 2026-06-06 (round 21) | Correctness Critic (#1) checklist | Extended round 10/20 guard principle to the full set of scalar field comparisons in `diffSchemaItems`: format, enum, nullable, and constraint fields must ALSO be inside the `if (bItems && cItems)` guard, not just additionalProperties/readOnly/writeOnly. The general rule: in any diff function that processes a "sub-schema entity" (items), ALL field-level comparisons for that entity except the primary type/existence detection must be inside `if (entity && entity)`. The type comparison stays outside as the primary detector; everything else moves inside. Probe: newly-added items schema with enum=[a,b,c] must NOT emit an `items-enum-changed` (BREAKING for request) event alongside the expected `items-type-changed`. | openapi-lens `diffSchemaItems`: format, enum, nullable, and 7 constraint fields were outside the `if (bItems && cItems)` guard. Newly-added items schemas with any of these fields would emit BREAKING events spuriously. Unified all scalar comparisons inside the guard (round 21 follows round 20's additionalProperties fix). +5 adversarial integration tests (3 no-spurious + 2 genuine-change-still-detected). **481/481 tests.** |
| 2026-06-06 (round 22) | Correctness Critic (#1) checklist | Extended the round-21 guard principle to `diffParameters` parameter items block. Every diff site that compares sub-schema entities (items, nested schemas) must apply the guard pattern: type comparison OUTSIDE, scalar fields INSIDE `if (both && exist)`. After fixing `diffSchemaItems` (rounds 20-21), audit every OTHER diff function that has a similar pattern. `diffParameters` had the same issue for parameter items scalar fields (format, enum, nullable, constraints). | openapi-lens `diffParameters`: parameter items format/enum/nullable/constraints compared outside the `if (bItems && cItems)` guard. A parameter changing from string to array with minLength on items emitted a spurious BREAKING constraint event. Fixed by inner guard. +2 adversarial tests. **483/483 tests.** |
