...
// ─── Round 119: numeric maximum null-transitions on request/response items + value tightening ──
// maximum on request/response items schemas has NEVER been tested — only parameter-items
// (R118-5) and string maxLength (various earlier rounds). These 5 tests cover:
//   request items: null→max (BREAKING), max→null (INFO)
//   response items: null→max (INFO), max→null (BREAKING)
//   request items minimum value tightening 1→5 (BREAKING — first value→value numeric path)
describe("Round 119 — numeric maximum null-transitions on request/response items + min value tightening", () => {
  it("(R119-1) request array items maximum added (null→200) is BREAKING — elements >200 now rejected", () => {
    // requestConstraintSeverity max-sense: before === null → BREAKING
    // Previously any integer element value was valid; clients sending >200 now get 422.
    function makeReqItemsMaxSpec(maxLine: string): string {
      return `
openapi: "3.0.3"
info: {title: T, version: "1"}
paths:
...
`;
    ...
    const changes = analyzeOpenApiDiff(makeReqItemsMaxSpec(""), makeReqItemsMaxSpec("maximum: 200"));
  ...
  it("(R119-2) request array items maximum removed (200→null) is INFO — constraint relaxed", () => {
    // requestConstraintSeverity max-sense: after === null → INFO
    // Server now accepts elements >200; clients that only sent ≤200 are unaffected.
    function makeReqItemsMaxSpec(maxLine: string): string {
      return `
openapi: "3.0.3"
info: {title: T, version: "1"}
...
`;
    ...
    const changes = analyzeOpenApiDiff(makeReqItemsMaxSpec("maximum: 200"), makeReqItemsMaxSpec(""));
    const c = changes.find((x) => x.type === "request-schema-items-constraint-changed" && String(x.location).endsWith(".maximum"));
    expect(c).toBeDefined();
    expect(c?.severity).toBe("INFO");
    expect(c?.before).toBe(200);
    expect(c?.after).toBeNull();
  });

  it("(R119-3) response array items maximum added (null→500) is INFO — server now guarantees bounded elements", () => {
    // responseConstraintSeverity max-sense: before === null → INFO
    // Server adds an upper-bound guarantee for array elements — clients gain knowledge.
    function makeRespItemsMaxSpec(maxLine: string): string {
      return `
openapi: "3.0.3"
info: {title: T, version: "1"}
paths:
...
`;
    ...
    const changes = analyzeOpenApiDiff(makeRespItemsMaxSpec(""), makeRespItemsMaxSpec("maximum: 500"));
    const c = changes.find((x) => x.type === "response-schema-items-constraint-changed" && String(x.location).endsWith(".maximum"));
    expect(c).toBeDefined();
    expect(c?.severity).toBe("INFO");
    expect(c?.before).toBeNull();
    expect(c?.after).toBe(500);
  });

  it("(R119-4) response array items maximum removed (500→null) is BREAKING — server may now return unbounded elements", () => {
    // responseConstraintSeverity max-sense: after === null → BREAKING
    // Clients that allocated buffers for ≤500 may overflow when server returns larger values.
    function makeRespItemsMaxSpec(maxLine: string): string {
      return `
openapi: "3.0.3"
info: {title: T, version: "1"}
...
`;
    ...
    const changes = analyzeOpenApiDiff(makeRespItemsMaxSpec("maximum: 500"), makeRespItemsMaxSpec(""));
    const c = changes.find((x) => x.type === "response-schema-items-constraint-changed" && String(x.location).endsWith(".maximum"));
    expect(c).toBeDefined();
    expect(c?.severity).toBe("BREAKING");
    expect(c?.before).toBe(500);
    expect(c?.after).toBeNull();
  });

  it("(R119-5) request array items minimum tightened (1→5) is BREAKING — elements 1-4 now rejected", () => {
    // requestConstraintSeverity min-sense: after > before → BREAKING
    // First value→value numeric constraint test: raising the floor breaks clients sending 1-4.
    function makeReqItemsMinSpec(minVal: number): string {
      return `
openapi: "3.0.3"
info: {title: T, version: "1"}
paths:
...
`;
    ...
    const changes = analyzeOpenApiDiff(makeReqItemsMinSpec(1), makeReqItemsMinSpec(5));
    const c = changes.find((x) => x.type === "request-schema-items-constraint-changed" && String(x.location).endsWith(".minimum"));
    expect(c).toBeDefined();
    expect(c?.severity).toBe("BREAKING");
    expect(c?.before).toBe(1);
...
// ─── Round 121: numeric value→value loosening (INFO) + parameter-items value tightening ────────
// Covers the "else" INFO branches for request items min/max value→value, and the first
// value→value numeric tests for parameter-items constraints.
describe("Round 121 — numeric value→value INFO directions + parameter-items value tightening", () => {
  it("(R121-1) request array items minimum loosened (5→1) is INFO — constraint relaxed, more elements accepted", () => {
    // requestConstraintSeverity min-sense: else → INFO (after < before, not null-transition)
    // Server now accepts elements ≥1 instead of ≥5; existing callers sending ≥5 unaffected.
    function makeReqItemsMinSpec(minVal: number): string {
      return `
openapi: "3.0.3"
info: {title: T, version: "1"}
...
`;
    ...
    const changes = analyzeOpenApiDiff(makeReqItemsMinSpec(5), makeReqItemsMinSpec(1));
    const c = changes.find((x) => x.type === "request-schema-items-constraint-changed" && String(x.location).endsWith(".minimum"));
    expect(c).toBeDefined();
    expect(c?.severity).toBe("INFO");
    expect(c?.before).toBe(5);
  ...
  it("(R121-2) request array items maximum loosened (50→100) is INFO — more values now accepted", () => {
    // requestConstraintSeverity max-sense: else → INFO (after > before, not null-transition)
    // Removing the upper constraint for elements 51-100; all existing callers remain valid.
    function makeReqItemsMaxSpec(maxVal: number): string {
      return `
openapi: "3.0.3"
info: {title: T, version: "1"}
...
`;
    ...
    const changes = analyzeOpenApiDiff(makeReqItemsMaxSpec(50), makeReqItemsMaxSpec(100));
    const c = changes.find((x) => x.type === "request-schema-items-constraint-changed" && String(x.location).endsWith(".maximum"));
    expect(c).toBeDefined();
    expect(c?.severity).toBe("INFO");
  ...
  it("(R121-3) response array items maximum tightened (50→100) is BREAKING — server guarantee weakened", () => {
    // responseConstraintSeverity max-sense: after > before → BREAKING
    // Server previously guaranteed elements ≤50; now allows up to 100 — clients may overflow.
...
`;
    ...
    const changes = analyzeOpenApiDiff(makeRespItemsMaxSpec(50), makeRespItemsMaxSpec(100));
    const c = changes.find((x) => x.type === "response-schema-items-constraint-changed" && String(x.location).endsWith(".maximum"));
    expect(c).toBeDefined();
    expect(c?.severity).toBe("BREAKING");
    expect(c?.before).toBe(50);
  ...
  it("(R121-4) parameter array items minimum tightened (1→5) is BREAKING — elements 1-4 now rejected", () => {
    // requestConstraintSeverity min-sense: after > before → BREAKING
    // First numeric value→value tightening test for parameter items.
...
`;
    ...
    const changes = analyzeOpenApiDiff(makeParamItemsMinSpec(1), makeParamItemsMinSpec(5));
    const c = changes.find((x) => x.type === "parameter-items-constraint-changed" && String(x.location).endsWith(".minimum"));
    expect(c).toBeDefined();
    expect(c?.severity).toBe("BREAKING");
    expect(c?.before).toBe(1);
  ...
  it("(R121-5) parameter array items maximum tightened (100→50) is BREAKING — elements 51-100 now rejected", () => {
    // requestConstraintSeverity max-sense: after < before → BREAKING
    // First numeric value→value tightening test for parameter maximum constraint.
...
`;
    ...
    const changes = analyzeOpenApiDiff(makeParamItemsMaxSpec(100), makeParamItemsMaxSpec(50));
    const c = changes.find((x) => x.type === "parameter-items-constraint-changed" && String(x.location).endsWith(".maximum"));
    expect(c).toBeDefined();
    expect(c?.severity).toBe("BREAKING");
    expect(c?.before).toBe(100);
...
// ─── Round 130: parameter-constraint value→value loosening + minProperties/maxProperties tightening ─
// Untested branches:
//   - requestConstraintSeverity min-sense: after < before → INFO (loosening: min decreases)
//   - requestConstraintSeverity max-sense: after > before → INFO (loosening: max increases)
//   - requestConstraintSeverity min-sense value→value: minProperties tightening (BREAKING)
//   - requestConstraintSeverity max-sense value→value: maxProperties tightening (BREAKING)
//   - requestConstraintSeverity min-sense value→value: minItems loosening (INFO)
describe("Round 130 — parameter-constraint value→value loosening + minProperties/maxProperties tightening", () => {
  function makeIntParamSpec(constraintLine: string): string {
    return `
openapi: "3.0.3"
info: {title: T, version: "1"}
...
`;
  ...
  it("(R130-1) query parameter minimum loosened (5→1) is INFO — constraint relaxed, more values accepted", () => {
    // requestConstraintSeverity min-sense: after (1) < before (5) → INFO (not tightening)
    const changes = analyzeOpenApiDiff(makeIntParamSpec("minimum: 5"), makeIntParamSpec("minimum: 1"));
    const c = changes.find((x) => x.type === "parameter-constraint-changed" && String(x.location).endsWith(".minimum"));
    expect(c).toBeDefined();
    expect(c?.severity).toBe("INFO");
    expect(c?.before).toBe(5);
  ...
  it("(R130-2) query parameter maximum loosened (50→100) is INFO — constraint relaxed, more values accepted", () => {
    // requestConstraintSeverity max-sense: after (100) > before (50) → INFO (not tightening)
    const changes = analyzeOpenApiDiff(makeIntParamSpec("maximum: 50"), makeIntParamSpec("maximum: 100"));
    const c = changes.find((x) => x.type === "parameter-constraint-changed" && String(x.location).endsWith(".maximum"));
    expect(c).toBeDefined();
    expect(c?.severity).toBe("INFO");
    expect(c?.before).toBe(50);
  ...
  it("(R130-3) array parameter minItems loosened (5→2) is INFO — relaxed: previously-rejected arrays now accepted", () => {
    // requestConstraintSeverity min-sense: after (2) < before (5) → INFO
    const changes = analyzeOpenApiDiff(makeArrayParamSpec("minItems: 5"), makeArrayParamSpec("minItems: 2"));
    const c = changes.find((x) => x.type === "parameter-constraint-changed" && String(x.location).endsWith(".minItems"));
    expect(c).toBeDefined();
    expect(c?.severity).toBe("INFO");
    expect(c?.before).toBe(5);
  ...
  it("(R130-4) object parameter minProperties tightened (1→3) is BREAKING — objects with 1-2 properties now fail", () => {
    // requestConstraintSeverity min-sense: after (3) > before (1) → BREAKING
    const changes = analyzeOpenApiDiff(makeObjectParamSpec("minProperties: 1"), makeObjectParamSpec("minProperties: 3"));
    const c = changes.find((x) => x.type === "parameter-constraint-changed" && String(x.location).endsWith(".minProperties"));
    expect(c).toBeDefined();
    expect(c?.severity).toBe("BREAKING");
    expect(c?.before).toBe(1);
  ...
  it("(R130-5) object parameter maxProperties tightened (10→3) is BREAKING — objects with 4-10 properties now fail", () => {
    // requestConstraintSeverity max-sense: after (3) < before (10) → BREAKING
    const changes = analyzeOpenApiDiff(makeObjectParamSpec("maxProperties: 10"), makeObjectParamSpec("maxProperties: 3"));
    const c = changes.find((x) => x.type === "parameter-constraint-changed" && String(x.location).endsWith(".maxProperties"));
    expect(c).toBeDefined();
    expect(c?.severity).toBe("BREAKING");
    expect(c?.before).toBe(10);
...
// ─── Round 131: parameter-constraint remaining value→value gaps ────────────────────────────
// Remaining untested parameter-constraint value→value branches:
//   maxItems loosening (after > before → INFO for max-sense)
//   minProperties loosening (after < before → INFO for min-sense)
//   maxProperties loosening (after > before → INFO for max-sense)
//   minLength tightening (after > before → BREAKING for min-sense) value→value
//   maxLength tightening (after < before → BREAKING for max-sense) value→value
describe("Round 131 — parameter-constraint remaining value→value loosening + string tightening", () => {
  function makeArrayParamSpec(constraintLine: string): string {
    return `
openapi: "3.0.3"
info: {title: T, version: "1"}
...
`;
  ...
  it("(R131-1) array parameter maxItems loosened (4→10) is INFO — previously-rejected arrays now accepted", () => {
    // requestConstraintSeverity max-sense: after (10) > before (4) → else branch → INFO
    const changes = analyzeOpenApiDiff(makeArrayParamSpec("maxItems: 4"), makeArrayParamSpec("maxItems: 10"));
    const c = changes.find((x) => x.type === "parameter-constraint-changed" && String(x.location).endsWith(".maxItems"));
    expect(c).toBeDefined();
    expect(c?.severity).toBe("INFO");
    expect(c?.before).toBe(4);
    expect(c?.after).toBe(10);
  });

  it("(R131-2) object parameter minProperties loosened (3→1) is INFO — objects with 1-2 properties now accepted", () => {
    // requestConstraintSeverity min-sense: after (1) < before (3) → else branch → INFO
    const changes = analyzeOpenApiDiff(makeObjectParamSpec("minProperties: 3"), makeObjectParamSpec("minProperties: 1"));
    const c = changes.find((x) => x.type === "parameter-constraint-changed" && String(x.location).endsWith(".minProperties"));
    expect(c).toBeDefined();
    expect(c?.severity).toBe("INFO");
    expect(c?.before).toBe(3);
  ...
  it("(R131-3) object parameter maxProperties loosened (3→10) is INFO — objects with 4-10 properties now accepted", () => {
    // requestConstraintSeverity max-sense: after (10) > before (3) → else branch → INFO
    const changes = analyzeOpenApiDiff(makeObjectParamSpec("maxProperties: 3"), makeObjectParamSpec("maxProperties: 10"));
    const c = changes.find((x) => x.type === "parameter-constraint-changed" && String(x.location).endsWith(".maxProperties"));
    expect(c).toBeDefined();
    expect(c?.severity).toBe("INFO");
    expect(c?.before).toBe(3);
  ...
  it("(R131-4) string parameter minLength tightened (3→8) is BREAKING — strings 3-7 chars now fail validation", () => {
    // requestConstraintSeverity min-sense: after (8) > before (3) → BREAKING
    // First value→value tightening test for minLength on parameter-constraint.
    const changes = analyzeOpenApiDiff(makeStringParamSpec("minLength: 3"), makeStringParamSpec("minLength: 8"));
  ...
  it("(R131-5) string parameter maxLength tightened (50→20) is BREAKING — strings 21-50 chars now fail validation", () => {
    // requestConstraintSeverity max-sense: after (20) < before (50) → BREAKING
    // First value→value tightening test for maxLength on parameter-constraint.
    const changes = analyzeOpenApiDiff(makeStringParamSpec("maxLength: 50"), makeStringParamSpec("maxLength: 20"));
