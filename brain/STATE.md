...
- **Build green:** **2073/2073 tests** (BidDiff 586/586 + openapi-lens 1487/1487). Rounds 54-115: continuous 5.7.5 bug-hunt + Phase 2 POLISH items. Round 97: +13 tests (minProperties/maxProperties in response headers + Swagger 2.0 header constraint tests + direction-aware header constraint message). Round 98: +7 tests (nested object property required-array changes — diffSchemaProperties recursion path, both request and response directions). Round 99: +7 tests (pattern→pattern non-null changes BREAKING for property/items/header/top-level). Round 100: +5 tests (parameter-items pattern constraint — null→pattern BREAKING, pattern→null INFO, pattern→pattern BREAKING, scalar parameter pattern→pattern, isolation). **POLISH N1 DONE** (baseline persistence + F1-1 fix: clearBaseline writes `undefined` not `""`). **POLISH N3 DONE** (progressive semantic line location + F2-1/F2-2/F2-5 fixes: searchFrom+1, no full-doc fallback, anchored JSON pattern). **POLISH T1 DONE** (direction-aware IAM + F3-1/F3-5 fixes: Deny statement analysis + assume_role_policy support). All 5.7.4 POLISH items complete. Round 96 (2026-06-06): response header schema constraints (minimum/maximum/minLength/maxLength/pattern/minItems/maxItems) were completely invisible to diff engine — added constraint comparison loop to `diffResponseHeaders` in diff.ts; added `response-header-constraint-changed` to OapiChangeType; added classify rule using responseConstraintSeverity (adding=INFO, removing=BREAKING); TYPE_STUBS updated; 11 adversarial end-to-end tests. +18 tests total (round 96: 11 + T1 critique fixes: 5 + N3 JSON test refinement: 1 + TYPE_STUBS: 1). Round 101: +1 test (object→scalar type change emits nested property removals; request-schema-property-removed is BREAKING). Round 102: +4 tests (items schema completely removed — simple type removal + object-with-properties removal fires type-changed + property-removed events via items recursion guard at diff.ts:467). Round 103: +3 tests (Terraform classify.test.ts — data store deletion Rule2+4, IAM replacement+widening Rule3+5, data store delete+create Rule3+4). Round 104: +3 tests (diagnosticProvider synthetic location strings — parameter(query:id) parenthesised segment, colon-inside-key graceful handling, responses[200] bracket notation). Round 105: +5 tests (Terraform classify.test.ts — IAM deletion Rule2+5 both fire; inline_policy field extraction widening+narrowing; egress CIDR block widening+narrowing). Round 106: +3 (top-level body schema format null transitions). Round 107: +3 (body enum missing directions). Round 108: +3 (property-type-changed null transitions). Round 109: +3 (items-format-changed null transitions). Round 110: +6 (items-enum-changed missing directions). Round 111: +5 (property-enum-changed missing directions). Round 112: +6 (parameter-enum and parameter-items-enum missing directions). Round 113: +5 (parameter-format/type null transitions + writeonly symmetric). Round 114: +5 (symmetric reverse directions for items readonly/writeonly/AP). Round 115: +5 (property-enum null transitions + AP reverse directions). Round 116: +5 (response property format→format BREAKING; items-nullable true→false INFO; body readonly/writeonly true→false INFO; items-format format→null BREAKING). Round 117: +5 (parameter-items-format format→format BREAKING; parameter-items-constraint null→minLength BREAKING; minLength→null INFO; minLength loosening INFO; response-items-constraint maxLength tightening INFO). Round 118: +5 (request-items-constraint minimum null→1 BREAKING; minimum 1→null INFO; response-items-constraint minimum null→0 INFO; minimum 0→null BREAKING; parameter-items-constraint maximum null→100 BREAKING — numeric min/max null-transitions on items schemas). Round 119: +5 (request-items-constraint maximum null→200 BREAKING; maximum 200→null INFO; response-items-constraint maximum null→500 INFO; maximum 500→null BREAKING; request-items-constraint minimum value tightening 1→5 BREAKING — first value→value numeric path on items schemas). Round 120: +5 (parameter-items-constraint minimum null→5 BREAKING; minimum 5→null INFO; maximum 100→null INFO; request-items-constraint maximum 100→50 BREAKING value→value; response-items-constraint minimum 5→1 BREAKING value→value). Round 121: +5 (request-items-constraint minimum 5→1 INFO loosening; maximum 50→100 INFO loosening; response-items-constraint maximum 50→100 BREAKING weakened guarantee; parameter-items minimum 1→5 BREAKING; parameter-items maximum 100→50 BREAKING). Round 122: +5 (response-items-constraint minimum 1→5 INFO; maximum 100→50 INFO; parameter-items minimum 5→1 INFO; maximum 50→100 INFO; response-header-constraint maximum 100→50 INFO — last missing tightening direction for header max). Round 123: **BUG FIX** + 5 tests — response-header-enum-changed severity rule used length comparison (`after.length > before.length`) instead of value-content comparison; same-length value swaps (e.g. "inactive"→"pending") incorrectly returned INFO when new values were added (BREAKING). Fixed classify.ts to use `added.length > 0 ? BREAKING : INFO`. Tests: value swap BREAKING, removal-only INFO, addition-only BREAKING, multi-swap BREAKING, no-change no-event. Round 124: +5 (response-header-constraint minLength null→8 INFO; 8→null BREAKING; maxLength null→50 INFO; 50→null BREAKING; minItems null→1 INFO — first tests for string/array constraint fields on response headers). Round 125: +5 (response-header-constraint maxItems null→5 INFO; 5→null BREAKING; minItems 1→null BREAKING; minLength 8→3 BREAKING value→value; maxLength 50→100 BREAKING value→value). Round 126: +5 (parameter-constraint minItems null→2 BREAKING; maxItems null→10 BREAKING; minItems 2→null INFO — first tests for array schema constraints on parameters; response-header-constraint minLength 3→8 INFO loosening; maxLength 100→50 INFO tightening). Round 127: +5 (parameter-constraint minimum 1→5 BREAKING; maximum 100→50 BREAKING — first numeric value→value tightening for parameters; maxItems 10→null INFO; minProperties null→1 BREAKING; minProperties 2→null INFO — first tests for object schema constraints on parameters). Round 128: +5 (parameter-items-constraint maxLength null→20 BREAKING; 20→null INFO; 100→30 BREAKING — first maxLength tests for parameter items; parameter-constraint maxProperties null→5 BREAKING; 5→null INFO — first maxProperties tests for parameter type). Round 129: +5 (Terraform IAM narrowing+replace — CRITICAL with "more restrictive" note when iamDirection=narrowing and severity already CRITICAL via Rule 3 replace; IAM narrowing pure update — NORMAL with "RESTRICT access" advisory; parameter-constraint minItems 2→5 BREAKING value→value tightening; maxItems 10→4 BREAKING value→value tightening; exclusiveMinimum/exclusiveMaximum known Phase 2 gap — no event emitted, test documents gap explicitly). Round 130: +5 (parameter-constraint minimum 5→1 INFO loosening; maximum 50→100 INFO loosening; minItems 5→2 INFO loosening; minProperties 1→3 BREAKING value→value tightening; maxProperties 10→3 BREAKING value→value tightening — closes all max/min loosening directions for numeric and array parameter constraints). Round 131: +5 (parameter-constraint maxItems 4→10 INFO loosening; minProperties 3→1 INFO loosening; maxProperties 3→10 INFO loosening; minLength 3→8 BREAKING value→value tightening; maxLength 50→20 BREAKING value→value tightening — closes all remaining parameter-constraint value→value loosening gaps and opens string length value→value tightening coverage). Round 132: +5 (parameter-items-constraint minLength 3→8 BREAKING value→value tightening; maxLength 30→100 INFO loosening; parameter-constraint minLength 8→3 INFO loosening; maxLength 20→50 INFO loosening; response-schema-items-constraint maxLength 50→100 BREAKING — server increases max length on response items, weakening consumer guarantee). Round 133: +5 (request-schema-items-constraint minLength 3→10 BREAKING tightening; maxLength 50→100 INFO loosening; response-schema-items-constraint minLength 10→2 BREAKING tightening; response-schema-property-constraint minLength 3→8 INFO increase; 8→3 BREAKING decrease — first value→value coverage for items+property minLength in response direction). Round 134: +5 (response-schema-property-constraint maxLength 50→100 BREAKING server-loosening; request-schema-property-constraint maxLength 100→50 BREAKING tightening; 50→100 INFO loosening; response body minItems 3→1 BREAKING — server weakens floor; response body maxItems 10→20 BREAKING — server weakens ceiling). Round 135: +5 (request body minItems 5→1 INFO loosening; request body minProperties 2→5 BREAKING tightening; 5→2 INFO loosening; response body minItems 1→3 INFO server-strengthens; response body minProperties 3→1 BREAKING server-weakens — completes minItems/minProperties value→value coverage for both request and response body). Round 136: +5 (request body maxProperties 5→3 BREAKING; 3→5 INFO; response body maxProperties 5→3 INFO server-tightens; 3→5 BREAKING server-loosens; response body minProperties 1→3 INFO server-strengthens — completes maxProperties value→value for both directions, closes all min/maxProperties value→value gaps). Round 137: +5 (nested array items minItems null→2 INFO on response — first test for response-schema-items-constraint for minItems on nested array schema; request nested array maxItems 10→4 BREAKING; simultaneous minimum+maximum change on request body produces two independent BREAKING events; parameter array items object minProperties null→2 BREAKING — first parameter-items-constraint for object schema; simultaneous minItems+maxItems change on response array produces two independent BREAKING events). Round 138: +5 (cookie parameter minLength null→5 BREAKING — first cookie constraint test; cookie parameter minimum 1→10 BREAKING; maximum 50→200 INFO loosening; pattern→null INFO; 3-level deeply nested property minLength tightening detected through full recursion chain shipping.address.postalCode). Round 139: +5 (array-of-objects body property constraint changes via diffSchemaItems→diffSchemaProperties path — first tests for CONSTRAINT changes on properties of array-of-objects schemas; request array-of-objects property minLength null→3 BREAKING; minimum 1→10 BREAKING; minimum 10→1 INFO loosening; response array-of-objects property minimum 10→3 BREAKING server-weakens; maximum removed value→null BREAKING server drops guarantee — route: diffSchemaItems detects items.properties, calls diffSchemaProperties which runs constraint loop). Round 140: +5 (array-of-objects body required-field + nullable/format via items path — diffSchemaItems→diffSchemaRequiredFields request path had never been tested; request array-of-objects required field added BREAKING; required field removed INFO; response array-of-objects required field added INFO; request items property format null→uuid BREAKING via diffSchemaItems→diffSchemaProperties→format; response items property nullable false→true BREAKING via diffSchemaItems→diffSchemaProperties→nullable). Round 141: +5 (array-of-objects properties writeOnly/readOnly/enum/additionalProperties via items path — response items property writeOnly false→true BREAKING; request items property readOnly false→true BREAKING; request items property enum null→[...] BREAKING; response items property enum [...]→null BREAKING; request items property additionalProperties closed BREAKING — all 5 tests exercise diffSchemaItems→diffSchemaProperties→specific-field-handler paths that had no prior coverage). Round 142: +5 (array-of-objects + nested combinations — request array-of-objects nested object property constraint changed BREAKING (2-level: items→address→zip.minLength); response array-of-objects nested object property removed BREAKING (2-level: items→details→code); request array-of-objects property with own items type changed BREAKING (items→tags→tags.items.type); response array-of-objects property items constraint changed (items→labels→labels.items.maxLength); request array-of-objects nested required field added BREAKING (2-level: items→contact.required[email])). Round 143: +5 (HEAD/OPTIONS/TRACE HTTP method coverage — HTTP_METHODS constant in parser.ts includes head/options/trace but all prior test rounds only targeted GET/POST/PUT/DELETE/PATCH; R143-1: HEAD endpoint removed BREAKING; R143-2: OPTIONS endpoint added INFO; R143-3: TRACE method parameter constraint change BREAKING; R143-4: HEAD method response status removed BREAKING; R143-5: OPTIONS method required parameter added BREAKING via adjustAddedRequiredParam — closes all 8 HTTP method coverage gaps). Round 144: +5 (flattenAllOf scalar-field inheritance + Swagger 2.0 path-level body + security scope union — three untested parser.ts code paths; R144-1: allOf member providing format inherited via flattenAllOf line 111; R144-2: allOf member providing nullable:true inherited via line 112 — response false→true BREAKING; R144-3: allOf member with additionalProperties:false inherited via line 117 — request schema closed BREAKING; R144-4: Swagger 2.0 path-level body parameter inherited when operation has no body param (buildSwagger2RequestBody pathLevelParams path, parser.ts line 453); R144-5: parseSecurityRequirements else branch — same scheme in two OR security entries scopes unioned, scope addition BREAKING). Round 145: +5 (flattenAllOf remaining scalar inheritance — readOnly from allOf member BREAKING on request (parser.ts line 113); writeOnly from allOf member BREAKING on response (line 114); items schema from allOf member — items type change detected (line 115); enum from allOf member — enum removal from response BREAKING (line 116); numeric constraint maximum from allOf member — request max decrease BREAKING (lines 119-121, completes the numericConstraints loop coverage for allOf)). Round 146: +5 (parser edge cases — Swagger 2.0 top-level #/parameters/X shared parameter ref resolution via parseSharedParameters+parseParameter regex `(components\/parameters|parameters)` branch (R146-1); OAS 3.x path-level parameter inherited by operations via pathLevelParams in parseParameters merge map (R146-2); OAS 3.x op-level parameter overrides path-level same-named param — op processed second in merge, suppresses path-level duplicate to produce single change (R146-3); resolveLocalRef `def === undefined` returns {} for matching regex but missing schema name (R146-4); detectVersion fallback when no openapi/swagger field returns "3.0" (R146-5)). Round 147: +5 (parser edge cases: implicit required + 3-hop $ref chain + malformed items + allOf items conflict + empty spec — R147-1: path parameter with no explicit required field is implicitly required=true (parser.ts line 238 `(inVal === "path")`); removal is BREAKING; R147-2: 3-hop $ref chain A→B→C fully resolved transitively via nested resolveLocalRef+normalizeSchema calls — type change detected end-to-end; R147-3: malformed items value (non-object `items: 42`) normalized to empty schema via normalizeSchema's `typeof raw !== "object"` guard — no crash, no diff event; R147-4: allOf with two members each providing items — first member's items win via flattenAllOf line 115 `merged.items = merged.items ?? m.items`; R147-5: spec with no paths produces zero diff changes — empty spec edge case). Round 148: +5 (Swagger 2.0 definitions allOf + OAS 3.1 only-null type + header $ref missing + allOf multi-required + empty security both sides — R148-1: Swagger 2.0 `definitions:` allOf composition with `$ref: "#/definitions/X"` — first test using `#/definitions/` regex branch in `resolveLocalRef` within allOf context; base property type change propagates to inheriting schema BREAKING via parseDefinitions→normalizeSchema→flattenAllOf; R148-2: OAS 3.1 `type: ["null"]` (only-null array) — `nonNullType=undefined` → no `schema.type` set but `schema.nullable=true` synthesized; changing to `["string","null"]` emits request-schema-type-changed (null→"string" BREAKING) but no nullable-changed (already true on both sides); R148-3: response header `$ref: "#/components/headers/NonExistentHeader"` — headerLookup[X] undefined → effective={} → normalizeSchema(undefined) = {} → schema:null; no crash, no events for identical spec; R148-4: allOf with two members each contributing different required fields — loop accumulation: after member 1 result.required=["id"], after member 2 result.required=["id","name"]; removing "name" from required in current detects response-schema-field-required-removed BREAKING; R148-5: both operations have `security: []` — parseSecurityRequirements([])={} (truthy empty object, not null) — diffOperationSecurity receives two {} objects → `!bSec && !cSec` = false (both truthy) → falls through loop with empty keys → no events). Round 149: +5 (classify.ts — analyzeIamDirection edge cases; R149-1: Action array containing `"*"` detected via `Array.isArray(action) && action.includes("*")` branch at line 124 — first test of array wildcard form (vs scalar `"*"` already tested); R149-2: extractIamStatements JSON parse failure — catch fires for all three fields (policy+assume_role_policy+inline_policy), returns null → CIDR also null → "unknown"; R149-3: doc["Statement"] non-array in first field → `Array.isArray(statements)` = false → loop continues → second field (assume_role_policy) has valid array used for analysis → "widening" detected; R149-4: CIDR count equal before/after → `afterCidrs === beforeCidrs` (both 2) → falls through to `return "unknown"` at classify.ts line 139 — first test of equal-CIDR-count path; R149-5: ipv6_cidr_blocks inside egress blocks — nested CIDR loop `for (const cidrField of ["cidr_blocks","ipv6_cidr_blocks"])` inside egress branch, first test combining egress+ipv6_cidr_blocks specifically).
