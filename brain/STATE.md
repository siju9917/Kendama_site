...
- **Build green:** **1769/1769 tests** (BidDiff 586/586 + openapi-lens 1183/1183). Rounds 54-60: cross-level matrices + security gap + OAS-3.1 normalization + cross-version comparison. Round 61: +25 tests (Terraform resource expansion). Round 62: content-type mismatch Phase-2 gap + operation-id all-directions + parameter-added edge cases (+10). Round 63: Terraform multi-rule interactions + parser output sensitivity (+8). Round 64: MAX_PROPERTY_DEPTH=5 boundary tests (+4). Round 65: parseOapiSpec error handling — empty string, malformed YAML, array/number root, JSON fast-path (+5). Round 66: extractContentSchema priority shift Phase 2 gap + isTerraformPlanJson edge cases + data store CREATE NORMAL guard (+9). Round 67: oneOf/anyOf Phase 2 gap + Swagger 2.0 schemes[]/basePath/host server URL edge cases (+8). Round 68: property type removal (type→null=BREAKING), implicit-object "(object)" fallback in before/after values, Swagger 2.0 absent-schemes defaults to https://, absent-host returns empty servers list (+6). Round 69: BUG FIX — renderOutputRow double-escape (esc called twice on actionText when sensitive=false; &amp; became &amp;amp; in output); 5 new WebView esc() completeness tests (double-quote, location HTML-injection, server-added Spec-level) + 1 Terraform double-escape regression test (+6). Round 70: response-header-enum null-transition paths (adding enum constraint = INFO, removing = BREAKING); Terraform webview type/terraformVersion HTML injection (+4). Round 71: response-header-type-changed null-transitions — type removed (string→absent=BREAKING), type added (absent→string=INFO); these classify rules were uncovered by any end-to-end spec test (+2). Round 72: parameter-items-type-changed (null→type=BREAKING) first end-to-end spec test; top-level request body pattern constraint added (null→pattern=BREAKING) and removed (pattern→null=INFO) — diffSchemaTopLevelConstraints pattern path never previously exercised via spec (+3). Round 73: top-level response body pattern constraint — adding (null→pattern=INFO, server adds guarantee) and removing (pattern→null=BREAKING, clients relied on guarantee); mirror of Round 72 with inverted semantics (+2). Round 74: top-level response body minimum constraint — all three responseConstraintSeverity min-sense paths: adding (null→5=INFO), decreasing (10→2=BREAKING), removing (5→null=BREAKING) (+3). Round 75: top-level response body maximum constraint — all three responseConstraintSeverity max-sense paths: adding (null→100=INFO), raising (100→200=BREAKING), removing (100→null=BREAKING) (+3). Round 76: top-level request body minimum/maximum null-transitions — adding minimum (null→1=BREAKING), removing minimum (1→null=INFO), adding maximum (null→100=BREAKING), removing maximum (100→null=INFO); these paths were only tested at property level, never at top-level body schema scope (+4). Round 77: response body top-level minLength/minItems null-transitions — all 4 paths (null→5=INFO, 5→null=BREAKING for minLength; null→3=INFO, 3→null=BREAKING for minItems); property-level tests existed but top-level body scope was unexercised (+4). Round 78: Terraform classify multi-rule interactions — data store + replace fires BOTH Rule 3 (REPLACED) and Rule 4 (data store) reasons; IAM + replace fires BOTH Rule 3 and Rule 5 (IAM) reasons; data store + delete fires Rule 2 + Rule 4; empty actions [] characterization (NORMAL with in-place reason) (+4). Round 79: Terraform parser output_changes sensitivity gaps — before_sensitive:{password:true} (partial object=sensitive), before_sensitive:false (not sensitive), before_sensitive:{} (empty object=sensitive), output_changes:[] (array guard=empty), entry.actions as string (not array=skipped) (+5). Round 80: request body minLength null-transitions (null→5=BREAKING, 5→null=INFO) + response body maxLength null-transitions (null→100=INFO, 100→null=BREAKING) end-to-end; plus classify "other" bucket dead-code characterization (requestConstraintSeverity/responseConstraintSeverity with unknown field name → INFO) (+6). Round 81: request body top-level maxLength null-transitions (null→50=BREAKING, 50→null=INFO) + response body top-level maxItems null-transitions (null→10=INFO, 10→null=BREAKING); maxLength at request top-level and maxItems at response top-level were both untested at null-transition paths (+4). Round 82: request body top-level minItems null-transitions (null→3=BREAKING, 3→null=INFO) + response body top-level minProperties null-transitions (null→2=INFO, 2→null=BREAKING); minItems at request body top-level and minProperties everywhere were completely untested (+4). Round 83: request body top-level maxItems null-transitions (null→10=BREAKING, 10→null=INFO) + response body top-level maxProperties null-transitions (null→5=INFO, 5→null=BREAKING); maxProperties had never been tested at any level (+4). Round 84: request body top-level minProperties (null→2=BREAKING, 2→null=INFO) + maxProperties (null→5=BREAKING, 5→null=INFO) null-transitions; completes the full 9-constraint × null-transition matrix for request body top-level body schema scope (+4). Round 85: top-level body constraint VALUE-CHANGE (non-null) branches — request minimum decrease (5→1=INFO), request maximum decrease (100→50=BREAKING) + increase (50→100=INFO), response minimum increase (2→10=INFO), response maximum decrease (200→100=INFO); all 5 previously untested at top-level body scope (+5). Round 86: pattern value-change (^[a-z]+$→^[A-Z]+$=BREAKING for request and response) + request body minLength value decrease (10→3=INFO) + response body maxLength value decrease (100→50=INFO); exercises non-null both-sides pattern arms and min/max-sense loosening paths at top-level body scope (+4). Round 87: items-level constraint null-transitions — adding minLength to request array items (null→5=BREAKING), removing (5→null=INFO); adding minLength to response array items (null→5=INFO), removing (5→null=BREAKING); only value-to-value items constraints were previously tested end-to-end (+4). Round 88: parameter constraint null-transitions — adding minimum (null→1=BREAKING), removing minimum (1→null=INFO), adding maximum (null→100=BREAKING), removing maximum (100→null=INFO); only value-to-value parameter constraint (100→50 BREAKING) was tested before (+4). Round 89: items maxLength null-transitions — request (null→50 BREAKING, 50→null INFO) + response (null→50 INFO, 50→null BREAKING); plus parameter constraint loosening — maximum increase (50→100 INFO) and minimum decrease (5→1 INFO); all 6 previously untested (+6). Round 90: parameter minLength/maxLength null-transitions for string parameters — all 4 paths (null→3=BREAKING, 3→null=INFO for minLength; null→50=BREAKING, 50→null=INFO for maxLength); all previous parameter constraint tests used integer schemas (+4). Round 91: items-level and parameter pattern constraint null-transitions — request array items (null→^[A-Z]{3}$ BREAKING, ^→null INFO), response array items (null→pattern INFO, pattern→null BREAKING), string parameter (null→pattern BREAKING, pattern→null INFO); all 6 were completely untested (+6). Round 92: items constraint value-change branches — request minLength decrease (10→3 INFO), response minLength increase (2→10 INFO), request maxLength decrease (100→50 BREAKING), response maxLength increase (50→100 BREAKING); completes the value-change direction matrix for items-level constraints (+4). Round 93: PUT method with request body constraint (null→1 BREAKING) — first PUT endpoint in all adversarial tests; header parameter maxLength (null→20 BREAKING); path parameter minLength (3→null INFO) — first path and header parameter constraint tests (+3). Round 94: PATCH method minimum (null→0 BREAKING) — first PATCH in adversarial tests; cookie parameter maxLength (null→32 BREAKING) — first cookie constraint test; multi-path isolation — changing one path produces zero changes on adjacent path (+3). Round 95: multi-response-code isolation — 200 minimum increase (1→5 INFO) detected, zero spurious 404 changes; simultaneous request body minimum (null→1 BREAKING) + response body maximum (null→100 INFO) both independently detected in one diff (+2).
