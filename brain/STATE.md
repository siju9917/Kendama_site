...
- **Build green:** **1662/1662 tests** (BidDiff 586/586 + openapi-lens 1076/1076). Rounds 54-60: cross-level matrices + security gap + OAS-3.1 normalization + cross-version comparison. Round 61: +25 tests (Terraform resource expansion). Round 62: content-type mismatch Phase-2 gap + operation-id all-directions + parameter-added edge cases (+10). Round 63: Terraform multi-rule interactions + parser output sensitivity (+8). Round 64: MAX_PROPERTY_DEPTH=5 boundary tests (+4). Round 65: parseOapiSpec error handling — empty string, malformed YAML, array/number root, JSON fast-path (+5). Round 66: extractContentSchema priority shift Phase 2 gap + isTerraformPlanJson edge cases + data store CREATE NORMAL guard (+9). Round 67: oneOf/anyOf Phase 2 gap + Swagger 2.0 schemes[]/basePath/host server URL edge cases (+8). Round 68: property type removal (type→null=BREAKING), implicit-object "(object)" fallback in before/after values, Swagger 2.0 absent-schemes defaults to https://, absent-host returns empty servers list (+6).
