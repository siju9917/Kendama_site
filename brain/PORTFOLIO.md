...
| **OpenAPI Breaking-Change Lens** (`products/openapi-lens/`) | VS Code extension that classifies breaking changes in OpenAPI schemas inline as you edit. | `build` (Phase 0 done — engine; awaiting Proposal #3 auto-proceed 2026-06-13) | Phase 0 complete: pure TypeScript diff engine, 68/68 tests, full classify pipeline (21 BREAKING/INFO rules, parse→diff→classify), YAML + JSON + Swagger 2.0 support. VS Code extension shell not yet built. | Phase 0 adversarial-clean: all 4 keyboard-guard classes tested, parser edge cases (path params, Swagger 2.0, $ref resolution, circular path-level param merge) tested. | Not yet — Proposal #3 auto-proceeds 2026-06-13; marketplace publisher reg needed (NEED #10). | $0 (pre-launch) | On `claude/intelligent-faraday-FnmJn` (current session); human gate: Proposal #3 and NEED #10. |
