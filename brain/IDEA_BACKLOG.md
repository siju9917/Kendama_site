...
| **D5. OpenAPI / API-spec breaking-change diff — VS Code extension + CI-free check** (**Phase 0 engine COMPLETE 2026-06-06 — 91/91 tests; Proposal #3 auto-proceeds 2026-06-13**) | VS Code Marketplace (largest dev marketplace) | API teams reviewing `openapi.yaml`/`swagger` changes for breaking edits | Proven-leaning (oasdiff / openapi-diff exist as CLIs — proves demand; an IDE-native, classified experience is the wedge) | Medium (7) — same breaking-change classification core | Score 636/1000. Phase 0 engine built at `products/openapi-lens/`. Phase 1 (VS Code extension scaffold) begins 2026-06-13. Monetization DIY via LemonSqueezy license keys. Distribution excellent. |
...
| **Dockerfile diff** | 6 | Moderate | 2 | 8 | DIFFERENT from Docker image diff above — a Dockerfile is structured text. Security-relevant changes between two Dockerfile versions: new `FROM` base image, `USER root`, newly exposed ports, added `RUN apt-get` installs, changed environment variables. Parser is trivial (line-oriented format). `hadolint` lints a single Dockerfile for best practices but does NOT compare two versions. No VS Code extension classifies Dockerfile change risk. Moderate gap — smaller audience than K8s/SQL, but same type of DevOps "what did this change break in our security posture?" pain. | **MEDIUM — genuine gap, easy parser, smaller audience than K8s/SQL** |
